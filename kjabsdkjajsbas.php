...
<body>
                    ...
                    if (isset($_POST['applyFilters'])) {
                        ...
                        if (!empty($customerSearch)) {
                            ...
                            if (!empty($customerIds)) {
                                ...
                                $courierCondition = '(' . implode(' OR ', $courierConditions) . ')';
                                
                                $sqry = mysqli_query($conn, "SELECT Courier_id FROM tbl_courier WHERE $courierCondition");
                                ...
                                if (!empty($courierIds)) {
                                    // Add the courier IDs to the main query
                                    $courierIdsString = implode("','", $courierIds);
                                    $query .= " AND Mastercart_id IN (SELECT Mastercart_id FROM tbl_courierassignment WHERE Courier_id IN ('$courierIdsString'))";
                                    
                                    if (!empty($startDate)) {
                                        $query .= " AND Mastercart_id IN (SELECT Mastercart_id FROM tbl_courierassignment WHERE Courier_id IN ('$courierIdsString') AND Courierassign_date >= '$startDate')";
                                    }
                                    if (!empty($endDate)) {
                                        $query .= " AND Mastercart_id IN (SELECT Mastercart_id FROM tbl_courierassignment WHERE Courier_id IN ('$courierIdsString') AND Courierassign_date <= '$endDate')";
                                    }
    ...
    <script>
        function printTable() {
            ...
            // Create a new window and set its content to the table
            var printWindow = window.open('', '', 'width=600');
            printWindow.document.open();
            printWindow.document.write('<html><head><title>Print Table</title>');

            // Include an external CSS file (optional)
            // printWindow.document.write('<link rel="stylesheet" type="text/css" href="styles.css">');

            // Add a <style> element within the <head> and define CSS rules
...
`);
            ...
            printWindow.document.write('<g>Re<spa>Tech</spa></g>');
            printWindow.document.write('<f>Your Trusted Source for Used Electronics</f>');
            printWindow.document.write('<h3 class="list">Sales Report</h3>');

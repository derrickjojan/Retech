...
<body>
    <div class="container">
        <div class="header">
            <h3>List of Courier</h3>
            <div class="">
                <a href="addcourier.php" class="btn"> Create New</a>
            </div>
        </div>
        <div class="table">
            <table>
                ...
                <thead>
                    <tr>
                        <th>No.</th>
                        <th>Courier id</th>
                        <th>Courier Username</th>
                        <th>Courier Name</th>
                        <th>Courier Address</th>
                        <th>Minimum Delivery Time</th>
                        <th>Maximum Delivery Time</th>
                        <th>Status</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $i = 1;
                    $qry = mysqli_query($conn, "SELECT * FROM `tbl_courier`");
                    while ($row = mysqli_fetch_assoc($qry)):
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo $row['Cs_Username'] ?>
                            </td>
                            <td>
                                <?php echo $row['Cs_name'] ?>
                            </td>
                            <td>
                                <?php echo $row['Cs_dist'] ?>,
                                <?php echo $row['Cs_city'] ?>
                            </td>


                            <td>
                                <?php echo $row['Cs_mindel'] . ' Days' ?>
                            </td>
                            <td>
                                <?php echo $row['Cs_maxdel'] . ' Days' ?>
                            </td>
                            <td>
                                <?php if ($row['Cs_Status'] == "1") {
                                    echo 'Active';
                                } else {
                                    echo 'Deactivated';
                                }
    ...
    <script>
        function printAndClose() {
            var printWindow = window.open("print_cour.php", "_blank");
        ...
        function exportToExcel() {
            fetch('viewcour.php') // Fetch data from viewcust.php
...
$currentDate = date('Y-m-d H:i:s');
        ...
        editLinks.forEach(function (editLink) {
            editLink.addEventListener('click', function (event) {
                ...
                var courierid = editLink.getAttribute('data-item-id');
                console.log(courierid); // Check if courierid has the correct value
                var url = 'managecourier.php?id=' + courierid;
        ...
        deleteLinks.forEach(function (deleteLink) {
            deleteLink.addEventListener('click', function (event) {
                event.preventDefault();
                var confirmation = confirm('Are you sure you want to deactivate this Courier?');
                if (confirmation) {
                    var courierid = deleteLink.getAttribute('data-item-id');
                    console.log(courierid); // Check if courierid has the correct value
                    var url = 'delcourier.php?id=' + courierid;
                    window.location.href = url;
                }
            });
        });
        ...
        addLinks.forEach(function (addLink) {
            addLink.addEventListener('click', function (event) {
                event.preventDefault();
                var confirmation = confirm('Are you sure you want to activate this Courier?');
                if (confirmation) {
                    var itemId = addLink.getAttribute('add-item-id');
                    console.log(itemId); // Check if itemId has the correct value
                    var url = 'adcourier.php?id=' + itemId;
                    window.location.href = url;
                }
            });
        });

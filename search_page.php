...
if (isset($_POST['add_to_cart'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $itemInfo = mysqli_query($conn, "SELECT stock FROM tbl_item WHERE Item_id = '$item_id_to_add'");
...
<body>
  ...
  <div class="container">
    <header>
      <nav>
        <div class="logo">
          <a href="home.php" class="fg-white cyber-glitch-2">Re<span>Tech</span></a>
        </div>
        <section class="search-form">

          <form action="" method="POST">
            <input type="text" class="code-block" name="search_box" placeholder="search products...">
            <div class="topnav">
...
if (isset($_POST['search_btn'])) {
  ...
  if (!empty($search_box)) {
      // Check if the search input contains a brand name
      $brandQuery = "SELECT Brand_id FROM tbl_Brand WHERE Brand_name LIKE '%$search_box%'";
      $brandResult = mysqli_query($conn, $brandQuery);
      
      if (mysqli_num_rows($brandResult) > 0) {
          // User entered a brand name, search by brand
          $brandIds = array();
          while ($brandRow = mysqli_fetch_assoc($brandResult)) {
              $brandIds[] = "'" . $brandRow['Brand_id'] . "'";
          }
      
          $brandIdList = implode(',', $brandIds);
          $select_products = mysqli_query($conn, "SELECT * FROM tbl_item WHERE Brand_id IN ($brandIdList) AND item_id IN (SELECT item_id FROM tbl_purchasechild WHERE stock != 0)");
      } else {
          // User didn't enter a brand name, search by item name
          $select_products = mysqli_query($conn, "SELECT * FROM tbl_item WHERE Item_name LIKE '%$search_box%' AND item_id IN (SELECT item_id FROM tbl_purchasechild WHERE stock != 0)");
      } 
